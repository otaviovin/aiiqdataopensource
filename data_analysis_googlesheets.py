...
# Flask - Web framework for building web applications and handling HTTP requests/responses
from flask import Flask, request, render_template, redirect, url_for, session, flash  # Core components for web routing, templates, sessions, and user messages

# PyMongo - Driver for interacting with MongoDB databases
from pymongo import MongoClient  # MongoDB client to perform database operations

# OS module - Used to work with environment variables and file paths
import os  # For handling environment variables and system-level operations

# Pandas - Data analysis and manipulation library
import pandas as pd  # Used for reading, cleaning, and manipulating tabular data (e.g., CSV files)

# Matplotlib - Plotting library for creating static, animated, and interactive visualizations
import matplotlib.pyplot as plt  # Main plotting interface for creating graphs and charts

# Seaborn - Statistical data visualization library built on top of Matplotlib
import seaborn as sns  # Provides high-level API for drawing attractive and informative plots

# Matplotlib base module - Sometimes required for setting backend or configurations
import matplotlib  # Base matplotlib package, sometimes used for configuration or backend setup

# IO - Input/output module for handling binary streams (e.g., images in memory)
import io  # For creating in-memory streams (e.g., image buffers)

# Base64 - Encoding/decoding binary data as Base64 strings
import base64  # Used to encode image data for embedding in HTML

# Dotenv - Loads environment variables from a .env file into the runtime
...
# Google Auth - Provides service account credentials for authenticating with Google APIs
from google.oauth2.service_account import Credentials  # Used to authorize access to Google Sheets

# Regular Expressions - For pattern matching and input validation
import re  # Useful for validating formats (e.g., email, ID, etc.)

# LangChain - Integration with OpenAI's ChatGPT models
from langchain_openai import ChatOpenAI  # Chat model for generating natural language responses

# LangChain - PromptTemplate for structuring prompts in a consistent format
from langchain.prompts import PromptTemplate  # Used to build and format prompts for language models
...
def analyze_data_googlesheets(df, user_input):
    """
    ...
    for column in numeric_columns:
        ...
        try:
            ...
            explanationai = generate_ai_analysis("Pie chart showing the proportional distribution of categories within the variable.", stats_summary, user_input)
            explanationsai.append(explanationai)
    ...
    # Build result dictionary to pass to the frontend
